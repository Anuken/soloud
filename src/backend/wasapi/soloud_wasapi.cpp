...
class MMNotificationClient : public IMMNotificationClient {
   public:
    MMNotificationClient(WASAPIData *aData)
        : IMMNotificationClient(), mRef(1), mData(aData){
    }
...
result wasapi_init(Soloud *aSoloud, unsigned int aFlags, unsigned int /*aSamplerate*/, unsigned int aBuffer, unsigned int /*aChannels*/){
    ...
    CoTaskMemFree(mixFormat);
    REFERENCE_TIME dur = static_cast<REFERENCE_TIME>(static_cast<double>(aBuffer) / (static_cast<double>(format.nSamplesPerSec) * (1.0 / 10000000.0)));
